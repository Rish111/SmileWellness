...
class FirstEntry: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Load Question
        pageTitle.text = "\(questions[0])..."
        
        // Setup auto past answers
        
        autoRecent1.setTitle("the weather", for: .normal) // to be updated for each page
        autoRecent2.setTitle("my family", for: .normal) // to be updated for each page
        autoRecent3.setTitle("my phone", for: .normal) // to be updated for each page
    ...
    func fetchSort() {
        ...
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: "Question1")
    ...
    func errorHighlightTextField(gratefulInput: UITextField) {
     ...
     // Write answer to database
        ...
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
        let entity = NSEntityDescription.entity(forEntityName: "Question1", in: context)
        ...
        currentAnswer1 = formatAnswer(question: questions[0], input: input!)
        do {
           try context.save()
          } catch {
           print("Failed saving")
    ...
    @IBAction func mrbutton1(_ sender: Any) {
        ...
        self.performSegue(withIdentifier: "1to2", sender: self)
    }
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if let text = gratefulInput.text, text.isEmpty {
            ...
            currentAnswer1 = formatAnswer(question: questions[0], input: gratefulInput.text!)
            do {
               try context.save()
                // Ensure answer is sent out
