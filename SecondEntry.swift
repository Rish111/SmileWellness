...
class SecondEntry: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Load Question
        pageTitle.text = "\(questions[1])..."
        
        // Setup auto past answers
        
        autoRecent1.setTitle("education", for: .normal) // to be updated for each page
        autoRecent2.setTitle("my friends", for: .normal) // to be updated for each page
        autoRecent3.setTitle("my dog", for: .normal) // to be updated for each page
      ...
      func fetchSort() {
          ...
          let request = NSFetchRequest<NSFetchRequestResult>(entityName: "Question2")
    ...
    func saveUp(button: UIButton) {
        ...
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
        let entity = NSEntityDescription.entity(forEntityName: "Question2", in: context)
        ...
        currentAnswer2 = formatAnswer(question: questions[1], input: input!)
        do {
           try context.save()
          } catch {
           print("Failed saving")
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if let text = gratefulInput.text, text.isEmpty {
            ...
            do {
               try context.save()
                // Ensure answer is sent out
                currentAnswer2 = formatAnswer(question: questions[1], input: gratefulInput.text!)
              } catch {
               print("Failed saving")
            }
            self.performSegue(withIdentifier: "2to3", sender: self)
