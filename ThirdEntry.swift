...
class ThirdEntry: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //Load Question
        pageTitle.text = "\(questions[2])..."
        
        // Setup auto past answers
        
        autoRecent1.setTitle("the gym", for: .normal) // to be updated for each page
        autoRecent2.setTitle("swimming", for: .normal) // to be updated for each page
        autoRecent3.setTitle("Netflix", for: .normal) // to be updated for each page
      ...
      func fetchSort() {
          ...
          let request = NSFetchRequest<NSFetchRequestResult>(entityName: "Question3")
    ...
    func saveUp(button: UIButton) {
        ...
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
        let entity = NSEntityDescription.entity(forEntityName: "Question3", in: context)
        ...
        currentAnswer3 = formatAnswer(question: questions[2], input: input!)
        do {
           try context.save()
          } catch {
           print("Failed saving")
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if let text = gratefulInput.text, text.isEmpty {
            ...
            do {
               try context.save()
                // Ensure answer is sent out
                currentAnswer3 = formatAnswer(question: questions[2], input: gratefulInput.text!)
              } catch {
               print("Failed saving")
            }
            self.performSegue(withIdentifier: "3to4", sender: self)
